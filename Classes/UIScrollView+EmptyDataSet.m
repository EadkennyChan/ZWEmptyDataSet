...
- (UIView *)emptyDataContentView
{
    return self.emptyDataSetView.contentView;
}
...
- (void)dzn_reloadEmptyDataSet
{
    ...
    if (([self dzn_shouldDisplay] && [self dzn_itemsCount] == 0) || [self dzn_shouldBeForcedToDisplay])
    {
        ...
        if (!view.superview)
        {
            ...
            else if ([self isKindOfClass:[UICollectionView class]] && self.subviews.count > 1)
            {
                [self insertSubview:view atIndex:1];
            }
            else
        ...
        [UIView performWithoutAnimation:^{
            ...
            if ([self isKindOfClass:[UITableView class]] && self.subviews.count > 1)
            {
                ...
                frame.origin.y = tbv.tableHeaderView.frame.size.height;
